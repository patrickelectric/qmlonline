class QmlOnline {
    ...
    loadWebASM() {
        console.log("Loading WebASM")
        const self = this
        var qtLoader
        qtLoader = QtLoader({
            //path: "https://qmlonline.kde.org/",
            containerElements: [self.canvas],
            ...
            showCanvas: function () {
                ...
                qtloader.module().instantiateWasm = function (imports, successCallback) {
        ...
        var intervalID = setInterval(
            function () {
                // console.log(`Waiting for webassembly to load... ${this.qtLoader}`)
                if (this.qtLoader == undefined || this.qtLoader.module == undefined) {
                    // console.warn("Module not loaded")
                    return
                }

                if (this.qtLoader && this.qtLoader.module && !this.qtLoader.module.printErr) {
                    this.qtLoader.module.printErr = function (text) {
                        // qml message
                ...
                if (this.qtLoader.module !== undefined) {
                    clearInterval(intervalID);
                    this.qtLoader.module.self().setCode(
                        "import QtQuick 2.7; \
